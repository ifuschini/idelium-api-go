#!/usr/bin/env python3
"""Build the CLI smoke-test target plan from route ownership contracts."""
...
SCHEMA_VERSION = 1
CONSUMER_ID = "idelium-cli"
OWNER_BASE_URL_ENV = {
    "laravel": "IDELIUM_CLI_SMOKE_LARAVEL_BASE_URL",
    "go": "IDELIUM_CLI_SMOKE_GO_BASE_URL",
}
SAFE_READ_METHODS = {"GET", "HEAD", "GET|HEAD"}


class SmokeTargetError(ValueError):
    """Report structural smoke-target errors without exposing runtime values."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-json", type=Path, default=Path("docs/contracts/cli-smoke-targets.json"))
    parser.add_argument("--output-md", type=Path, default=Path("docs/contracts/cli-smoke-targets.md"))
...
def execution_mode(method: str, operation_kind: str | None) -> str:
    if method in SAFE_READ_METHODS and operation_kind == "read":
        return "configuration-read"
    return "result-reporting-write"
...
def build_targets(consumer_map: dict[str, Any], ownership_matrix: dict[str, Any]) -> dict[str, Any]:
    ...
    for route in consumer_map.get("routes", []):
        ...
        if owner not in OWNER_BASE_URL_ENV:
            ...
            continue
        mode = execution_mode(method, ownership.get("operation_kind"))
        targets.append(
            {
                "route_id": identifier,
                "method": method,
                ...
                "execution_mode": mode,
                "requires_runtime_credential": route.get("authentication_mode") == "api-key",
            }
        )
    ...
    return {
        ...
        "target_policy": {
            ...
            "runtime_credentials_are_external": True,
        },
        "summary": {
