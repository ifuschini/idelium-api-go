# Platform browser read migration

Wave 3 moves the read-only browser endpoint from Laravel to the Go API while
browser mutations remain on Laravel for Wave 6.

## Owned route

| Method | Public path | Go handler | Rollout status |
| --- | --- | --- | --- |
| `GET` | `/api/admin/platforms/browsers/{idOs}` | `platforms.Handler.Browsers` | `go-owned` |

`POST` and `PUT` on `/api/admin/platforms/browsers` stay `laravel-owned`
because they mutate the catalog and require the Wave 6 mutation contract.

## Data access contract

The Go route preserves Laravel's `EnterpriseGridResponse` shape for browsers
scoped to an operating-system row:

- `idOs` is validated as a positive integer at the HTTP boundary;
- without `page` or `pageSize`, the response is the legacy array of browser rows;
- with `page` or `pageSize`, the response is `{ "data": [...], "meta": {...} }`;
- each row exposes `id`, `name`, `idOs`, `created_at`, and `updated_at`;
- `sort` is allowlisted to `id`, `name`, `created_at`, and `updated_at`;
- the default sort is `name asc`, matching the Laravel controller;
...
```sql
SELECT id, name, idOs, created_at, updated_at
FROM browsers
WHERE idOs = ?
...
Browsers are global platform catalog metadata under a parent operating system,
...
when a browser is attached to project-scoped platform targets or execution
results.
...
route the safe `GET` to `IDELIUM_WEB_SMOKE_GO_BASE_URL`, while browser mutations
...
1. Change `GET|HEAD /api/admin/platforms/browsers/{idOs}` in
...
3. Route `GET /api/admin/platforms/browsers/{idOs}` back to Laravel at the
   gateway.

No database migration is required because the Go route reads the existing
Laravel-compatible `browsers` table.
